    ...
    <div className="mt-3 space-y-2.5 text-left">
      ...
      {backup && (
        <button
          ...
          className="w-full rounded-miro border border-miro-border bg-white p-3.5 hover:border-miro-purple/30 hover:shadow-miro transition-all text-left"
        >
          <p className="text-[10px] uppercase tracking-wider text-miro-ink-subtle font-semibold mb-1">
            Backup
          </p>
          <p className="font-medium text-sm text-miro-ink">{backup.name}</p>
          <p className="text-xs text-miro-ink-muted">{backup.neighborhood}</p>
        </button>
      )}
      ...
      <div className="rounded-miro border border-miro-purple/20 bg-miro-purple-soft p-3.5">
        ...
        <p className="text-xs text-miro-ink font-medium">{transit.summary}</p>
        <p className="text-xs text-miro-ink-muted flex items-center gap-2 mt-2">
        ...
        <ol className="mt-2 space-y-1">
          {transit.steps.map((step, i) => (
            <li
              key={i}
              className="text-[11px] text-miro-ink-muted flex items-start gap-2"
            >
              <Bus className="w-3 h-3 shrink-0 mt-0.5 text-miro-ink-subtle" />
              {step}
            </li>
          ))}
        </ol>
